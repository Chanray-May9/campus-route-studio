...
很抱歉安卓版本（包括模拟器）目前存在无法定位的bug，具体表现为启动辅助软件之后乐跑软件上面定位虽然发生更新但没有出现移动，但我们仍然保留了安卓版本的安装包但是安装之后可能无法正常使用，我们对用户造成的不便表达十分歉意，如果您有解决方法欢迎Issues或Fork。   但是作案通过数据线连接手机仍然可以正常使用。
